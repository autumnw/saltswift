...
def exists(name, type='file'):
    '''
    type = file | dir | symlink
    '''
    if os.path.exists(name) == False:
        return False
    if type == 'file' and os.path.isfile(name):
        return True
    elif type == 'dir' and os.path.isdir(name):
        return True
    elif type == 'symlink' and os.path.islink(name):
        return True
    return False

def touch_r(name, atime=None, mtime=None):
    '''
    Based on touch(), just check if the parent directory exists or not. 
    If no, create directory firstly.
    '''
    tmp_list = name.rsplit(os.sep, 1)
    if len(tmp_list) == 2:
        parent_dir = tmp_list[0]
        if os.path.exists(parent_dir) == False:
            os.makedirs(parent_dir, 0755)
    return touch(name, atime, mtime)
