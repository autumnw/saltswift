...
class SwiftWriteRotatingFileHandler(logging.handlers.RotatingFileHandler):

    def _open(self):
        prevumask=os.umask(0o001)
        rtv=logging.handlers.RotatingFileHandler._open(self)
        os.umask(prevumask)
        return rtv

    def format(self, record):
        slf = SwiftLogFormatter()
        return  slf.format(record)
...
def get_logger(conf, name=None, log_to_console=False, log_route=None,
    ...
    logging.SwiftWriteRotatingFileHandler =  SwiftWriteRotatingFileHandler # Bind your module to


    # Use a logging configuration file for all settings...
    if os.path.exists('/etc/swift/logging.conf'):
        logging.config.fileConfig('/etc/swift/logging.conf')
    else:
        raise RuntimeError('Unable to locate specified logging '
                            'config file: /etc/swift/logging.conf' )
